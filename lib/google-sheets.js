...
export async function updateRow(sheetName, rowKey, rowValue, payload) {
  try {
    console.log(`[GoogleSheets] Attempting Update in ${sheetName} where ${rowKey}=${rowValue}`);
    const doc = await getDoc();
    const sheet = doc.sheetsByTitle[sheetName];
    if (!sheet) {
      console.error(`[GoogleSheets] Sheet not found: ${sheetName}`);
      return false;
    }
    const rows = await sheet.getRows();
    
    // Tìm hàng khớp với khóa chính
    const row = rows.find(r => String(r.get(rowKey)) === String(rowValue));
    
    if (row) {
      console.log(`[GoogleSheets] Found row to update. Keys in payload:`, Object.keys(payload));
      
      // Cập nhật từng ô một cách thủ công để đảm bảo tính ổn định của thư viện v4
      for (const key of Object.keys(payload)) {
        // Không cập nhật lại ID nếu nó nằm trong payload (Google đôi khi chặn việc sửa lại ID của hàng hiện có)
        if (key !== rowKey) {
          row.set(key, payload[key]);
        }
      }
    ...
    console.error(`[GoogleSheets] Error during updateRow in ${sheetName}:`, error);
    return false;
  }
}
...
export async function deleteRow(sheetName, rowKey, rowValue) {
  try {
    console.log(`[GoogleSheets] Attempting Delete in ${sheetName} where ${rowKey}=${rowValue}`);
    const doc = await getDoc();
    const sheet = doc.sheetsByTitle[sheetName];
    if (!sheet) return;
    const rows = await sheet.getRows();
...
export async function createRow(sheetName, payload) {
  try {
    console.log(`[GoogleSheets] Attempting Create in ${sheetName}`, payload);
    const doc = await getDoc();
    const sheet = doc.sheetsByTitle[sheetName];
    if (!sheet) {
